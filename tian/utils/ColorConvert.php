...
class ColorConvert {
	...
	public function hsb2rgbArr($hue, $saturation, $brightness) {
		...
		// echo $hue;
		if ($hue < 0 || $hue > 360) {
			\tian\log::w ( 'Argument $hue is not a number between 0 and 360' );
			return false;
		}
		$hue = $hue == 360 ? 0 : $hue;
		$saturation = $this->significantRound ( $saturation, 3 );
		if ($saturation < 0 || $saturation > 100) {
			\tian\log::w ( 'Argument $saturation is not a number between 0 and 100' );
			return false;
		}
		$brightness = $this->significantRound ( $brightness, 3 );
		if ($brightness < 0 || $brightness > 100) {
			\tian\log::w ( 'Argument $brightness is not a number between 0 and 100.' );
			return false;
		}
	...
	public function rgb2hsbArr($R, $G, $B) {
	...
	private function significantRound($number, $precision) {
		if (! is_numeric ( $number )) {
			tian\log::w ( 'Argument $number must be an number.' );
			return 0;
		}
		if (! is_int ( $precision )) {
			tian\log::w ( 'Argument $precision must be an integer.' );
			return 0;
		}
